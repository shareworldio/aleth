...
struct ErrorChannel: public LogChannel { static const char* name(); static const int verbosity = 0; static const bool debug = false; };
...
#if NLOG
#define clog_stream(X) (nlog(X)<<LOG_INFO)
#define cslog_stream(X) (nslog(X)<<LOG_INFO)
#else
#if NDEBUG
#define clog_stream(X) ((DEV_STATEMENT_IF(!(X::debug)) dev::LogOutputStream<X, true>())<<LOG_INFO))
#define cslog_stream(X) ((DEV_STATEMENT_IF(!(X::debug)) dev::LogOutputStream<X, false>())<<LOG_INFO))
#else
#define clog_stream(X) (dev::LogOutputStream<X, true>()<<LOG_INFO)
#define cslog_stream(X) (dev::LogOutputStream<X, false>()<<LOG_INFO)
#endif
#endif

// Simple cout-like stream objects for accessing common log channels.
// Dirties the global namespace, but oh so convenient...
#define cerror clog_stream(dev::ErrorChannel)
#define cdebug clog_stream(dev::DebugChannel)
#define cnote clog_stream(dev::NoteChannel)
#define cwarn clog_stream(dev::WarnChannel)
#define ctrace clog_stream(dev::TraceChannel)
