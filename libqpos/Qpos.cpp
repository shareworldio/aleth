...
bool Qpos::interpret(const NodeID &_nodeid, unsigned _id, RLP const& _r)
{
	unsigned msgType;
	try{
		msgType = _r[0][0].toInt();
	}catch(...){
		cdebug << "msgType err _id= " << _id << ",_nodeid=" << _nodeid;
		return true;
	}

	bool contain = m_miners.count(_nodeid);
	if(!contain){
		cdebug << "interpret _nodeid=" << _nodeid << ",_id=" << _id << ",msgType=" << msgType;
		return true;
	}
	...
	try{
		switch(msgType){
			case qposBlockVote:
				onBlockVote(_nodeid, _r);
				break;
			case qposBlockVoteAck:
				onBlockVoteAck(_nodeid, _r);
				break;
			case qposHeart:
				onHeart(_nodeid, _r);
				break;
			case qposVote:
				...
				break;
			case qposVoteAck:
				onVoteAck(_nodeid, _r);
				break;
			case qposBroadBlock:
				onBroadBlock(_nodeid, _r);
				break;
			default:
				break;
		}
	}catch(...){
		cwarn << "catchErrinterpret _nodeid=" << _nodeid << ",_id=" << _id << ",msgType=" << msgType;
	}
...
void Qpos::initEnv(class Client *_c, p2p::Host *_host, std::shared_ptr<EthereumCapability> _capability, BlockChain* _bc, bool _importAnyNode)
{
	m_importAnyNode = _importAnyNode;
	m_sigset = new signal_set(_host->ioContext(), QPOS_SIGNAL, QPOS_SIGNAL_REPORT);
	m_sigset->async_wait(boost::bind(&Qpos::signalHandler, this, _1, _2));

	//DumpStack();
	
	srand(utcTime());
	QposSealEngine::initEnv(_c, _host, _capability, _bc, _importAnyNode);
	...
	_c->setFilter([=](p2p::NodeID _nodeid, unsigned _id, RLP const&) -> bool{
...
void Qpos::onBroadBlock(const NodeID &_id, RLP const& _r)
{
	(void)_id;
...
void Qpos::onBlockVoteAck(const NodeID &_id, RLP const& _r)
{	
	...
	bool v = msgVerify(_id, m_blockBytes, mySign);
	...
	if(vote){
		m_idVoted[_id] = mySign;
	}else{
		m_idUnVoted.insert(_id);
	}
...
void Qpos::onBlockVote(const NodeID &_id, RLP const& _r)
{
	...
	bool verify = msgVerify(_id, blockBytes, mySign);
	...
	send(_id, data);
	cdebug << "hash=" << hash << ",mySign=" << mySign << ",blockBytes.size()=" << blockBytes.size() << ",vote=" << vote << ",_id=" << _id;
}
...
void Qpos::onHeart(const NodeID &_id, RLP const& _r)
{
	int64_t currentView = _r[0][1].toInt();

	if(m_isLeader && (currentView > m_currentView ||  (currentView == m_currentView && _id > id()))){
		m_isLeader = false;
	}
...
void Qpos::onVote(const NodeID &_id, RLP const& _r)
{
	int64_t currentView = _r[0][1].toInt();
	...
	cdebug << "_id=" << _id << ",currentView=" << currentView << ",m_currentView=" << m_currentView << ",vote=" << vote;
	send(_id, data);
}

void Qpos::onVoteAck(const NodeID &_id, RLP const& _r)
{
	bool vote = _r[0][1].toInt();
