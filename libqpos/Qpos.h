...
{
public:
	virtual bool interpret(const NodeID &_nodeid, unsigned _id, RLP const& _r);

	virtual bool shouldSeal(Interface*);

	virtual void initEnv(class Client *_c, p2p::Host *_host, std::shared_ptr<EthereumCapability> _capability, BlockChain* _bc, bool _importAnyNode) override;
...
protected:
	...
	void onBlockVoteAck(const NodeID &_id, RLP const& _r);
	void onBlockVote(const NodeID &_id, RLP const& _r);
	void onHeart(const NodeID &_id, RLP const& _r);
	void onVote(const NodeID &_id, RLP const& _r);
	void onVoteAck(const NodeID &_id, RLP const& _r);
	void voteTick();
	bytes authBytes();
	void broadBlock(bytes const& _bolck);
	void onBroadBlock(const NodeID &_id, RLP const& _r);
	void voteBlockEnd();
	void voteBlockBegin();
