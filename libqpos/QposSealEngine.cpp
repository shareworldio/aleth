...
void QposSealEngine::initEnv(class Client *_c, p2p::Host *_host, std::shared_ptr<EthereumCapability> _capability, BlockChain* _bc, bool _importAnyNode)
{
	(void)_importAnyNode;
...
void QposSealEngine::send(NodeID const& _id, RLPStream& _msg)
{
	if (auto h = m_host.lock())
		h->capabilityHost().sealAndSend(_id, _msg);
}
...
{	
	if (auto h = m_host.lock()){
		h->capabilityHost().foreachPeer("eth", [=](NodeID const& _peerID) {
			RLPStream msg(_msg);
			send(_peerID, msg);
			return true;
		});
...
/*
	m_host->foreachPeer("eth", [=](NodeID const& _peerID) {
		RLPStream msg(_msg);
        send(_peerID, msg);
        return true;
    });
...
bool QposSealEngine::interpret(const NodeID &_nodeid, unsigned _id, RLP const& _r) 
{
	(void)_nodeid;
	(void)_id;
	(void)_r;
	...
	for(auto it : _nodes){
		...
		try{
			if(property.count("ip") && property["ip"].length() > 0 && property.count("port") && property["port"].length() > 0){
				...
				cdebug << "spec=" << spec;
				m_p2pHost->addPeer(p2p::NodeSpec(spec), p2p::PeerType::Required);
			}
		}catch(...){
		}
	}
