...
::boost::log::record g_boost_log_record = (dev::g_debugLogger::get()).open_record();

/
typeof(::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), g_boost_log_record).stream()) get_stream(){
	::boost::log::record _boost_log_record = (dev::g_debugLogger::get()).open_record();
	return ::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), _boost_log_record).stream()
}

*/

boost::log::v2s_mt_posix::aux::record_pump<boost::log::v2s_mt_posix::sources::severity_channel_logger_mt<> > get_record(){
	for (::boost::log::record _boost_log_record_175 = (dev::g_debugLogger::get()).open_record(); !!_boost_log_record_175;){ 
		return ::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), _boost_log_record_175);
	}
	
	::boost::log::record _boost_log_record = (dev::g_debugLogger::get()).open_record();
	return ::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), _boost_log_record);
}
...
void QposClient::init(ChainParams const&, p2p::Host *_host) {
	...
	raft()->initEnv(this, _host, m_host.lock(), &m_bc, m_importAnyNode);
	ctrace << "Init QposClient success 11";

	Logger infoLogger{createLogger(VerbosityInfo, "net")};
	LOG(infoLogger) << "Init QposClient success";

	//auto foo(arg) -> decltype(XXMACRO){}

	//::boost::log::record _boost_log_record = (dev::g_debugLogger::get()).open_record();
	//::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), static_cast<::boost::log::record&&>((dev::g_debugLogger::get()).open_record())).stream() << LOG_INFO;
	//boost::log::v2s_mt_posix::sources::logging::record_ostream((dev::g_debugLogger::get()).open_record()) << "_blockNumber=++++++++++++++++++++++++++++++++++++++++++++++++++++++++";
	//::boost::log::record _boost_log_record = (dev::g_debugLogger::get()).open_record();
	//auto st = ::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), _boost_log_record);
	//ctrace << "********************************" << typeinfo(st);
	//(get_record().stream()<<LOG_INFO) << "_blockNumber=";

	/*
	for (::boost::log::record _boost_log_record_175 = (dev::g_debugLogger::get()).open_record(); !!_boost_log_record_175;){ 
		(::boost::log::aux::make_record_pump((dev::g_debugLogger::get()), _boost_log_record_175).stream()<<LOG_INFO("hello")) <<"_blockNumber=";
	}
	*/
}
